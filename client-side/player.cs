...
public player() { }
	...
	protected event PropertyChangeHandler<int> __indexChange;
	public Action OnIndexChange(PropertyChangeHandler<int> __handler, bool __immediate = true) {
		if (__callbacks == null) { __callbacks = new SchemaCallbacks(); }
		__callbacks.AddPropertyCallback(nameof(this.index));
		__indexChange += __handler;
		if (__immediate && this.index != default(int)) { __handler(this.index, default(int)); }
		return () => {
			__callbacks.RemovePropertyCallback(nameof(index));
			__indexChange -= __handler;
		};
	}
	...
	protected override void TriggerFieldChange(DataChange change) {
		switch (change.Field) {
			...
			case nameof(index): __indexChange?.Invoke((int) change.Value, (int) change.PreviousValue); break;
			default: break;
		}
	}
}
